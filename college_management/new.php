...
$result_count = "SELECT COUNT(*) As total_records FROM institution_details";
$result = $conn->query($result_count);
$row = $result->fetch_assoc();
$total_results = $row['total_records'];
...
$second_last = $total_no_of_pages - 1;
  ...
  $starting_limit = ($current_page - 1) * $total_records_per_page;
  $ending_limit = $offset + $total_records_per_page;
  // Retrieve the data using a SQL query with the starting and ending limits
  if (isset($_GET['search'])) {
    $search_term = $_GET['search'];
    $result_count = "SELECT * FROM institution_details WHERE Institution_Name LIKE '%$search_term%' OR State LIKE '%$search_term%' LIMIT $starting_limit, $ending_limit";
    $result = $conn->query($result_count);
  } else {
    $result_count = "SELECT * FROM institution_details LIMIT $starting_limit, $ending_limit";
    $result = $conn->query($result_count);
  }
